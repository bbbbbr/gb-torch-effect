...
void update_attribute_map_torch_effect(void) {
    ...
    if ((atr_cent_x != atr_cent_x_last) || (atr_cent_y != atr_cent_y_last)) {
        ...
        if (atr_cent_x > atr_cent_x_last) { // Moving Right
            fill_bkg_rect(atr_cent_x_last - TORCH_WIDTH_ST, atr_cent_y_last - TORCH_WIDTH_ST, 1, TORCH_HEIGHT, PAL_DRK);
        }
        else if (atr_cent_x < atr_cent_x_last) { // Moving Left
            fill_bkg_rect(atr_cent_x_last + TORCH_WIDTH_ST, atr_cent_y_last - TORCH_WIDTH_ST, 1, TORCH_HEIGHT, PAL_DRK);
        }

        if (atr_cent_y > atr_cent_y_last) { // Moving Down
            fill_bkg_rect(atr_cent_x_last - TORCH_WIDTH_ST, atr_cent_y_last - TORCH_WIDTH_ST, TORCH_WIDTH, 1, PAL_DRK);
        }
        else if (atr_cent_y < atr_cent_y_last) { // Moving Up
            fill_bkg_rect(atr_cent_x_last - TORCH_WIDTH_ST, atr_cent_y_last + TORCH_WIDTH_ST, TORCH_WIDTH, 1, PAL_DRK);
        }            
...
void main(void)
{
    ...
    while(1) {
        ...
        update_attribute_map_torch_effect();
